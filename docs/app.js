...
const DNODES = {
  ...
  rNC: { rec: { kind: "toolbox", method: "nc", badge: "NC ✓",
    ...
    scenario: { zh: "藥物X 情境：疾病嚴重度／就醫傾向（未測）同時影響用藥物X 與結果。用用藥前事件當陰性對照結果、另一個與就醫傾向相關但不影響結果的暴露當陰性對照暴露，偵測並校正 healthy-user 偏誤（見「NC」分頁 ①–⑥）。",
  ...
  rTRANS: { rec: { kind: "toolbox", method: "transport", badge: "可轉移性 ✓",
    ...
    scenario: { zh: "藥物X 情境：藥物X 的效果來自一場在某國成人做的 RCT，但你關心的是<b>本地長者</b>。用兩邊共同的效果修飾因子分布重新加權，把試驗估到的效果「轉」到本地長者族群。",
          ...
          forks: [
            { edge: { zh: "一次性／致命／慢性 → 世代與時點設計", en: "one-off / fatal / chronic → cohort & timing designs" },
              leaves: [
                ...
                { key: "rACC", cond: { zh: "有藥理相近的活性對照（用藥物X vs 對照藥）", en: "a similar active comparator (drug X vs comparator)" }, tag: "ACNU ✓", kind: "tb" },
...
function renderAlign() {
  ...
  const checklist =
    ...
    `<li><b>重要干擾因子都測到了嗎？</b>疾病嚴重度、健康服藥者（healthy-adherer）傾向有沒有被觀察到（否則是無法被測量的干擾因子）？</li>` +
    `</ul></details>`;
...
// ======================================================================
const BIAS_TYPES = [
  { key: "immortal", zh: "不死時間（Time Zero沒有對齊）", en: "Immortal time (time-zero misalignment)" },
...
const BIAS_SCENARIOS = [
  { s: { zh: "把「確診後<b>曾用過</b>藥物X」的人算成暴露組，<b>Time Zero設在確診日</b>、一路追蹤到死亡，再和從頭到尾沒用過藥的人比存活。", en: "Call people 'exposed' if they <b>ever started</b> drug X after diagnosis, put <b>time zero at the diagnosis day</b>, follow to death, and compare survival with never-users." },
    ...
    fix: { zh: "把暴露當<b>時變</b>，或改用<b>地標</b>／<b>複製-設限-加權</b>對齊Time Zero；對照改成<b>主動對照</b>解適應症混淆。", en: "Model exposure as <b>time-varying</b>, or use <b>landmark</b> / <b>clone-censor-weight</b> to align time zero; switch to an <b>active comparator</b> for the indication problem." }, fixMethod: "ccw" },
  ...
  { s: { zh: "用健保資料比較兩種同類藥物X，設計良好（新使用者、主動對照、Time Zero對齊），但健保<b>沒有檢驗值</b>（疾病嚴重度）。", en: "Compare two same-class drug-X options in claims with a good design (new-user, active comparator, aligned time zero) — but claims have <b>no lab values</b> (disease severity)." },
    ans: ["unmeasured"],
    why: { zh: "設計已消掉不死時間，主動對照也壓低適應症混淆；但「疾病嚴重度」沒被測到，若它同時影響選藥與結果，就是<b>無法被測量的干擾因子</b>。", en: "The design removes immortal time and the active comparator curbs confounding by indication; but severity is unmeasured — if it drives both drug choice and outcome, that is an <b>unmeasured confounder</b>." },
  ...
  { s: { zh: "比較「長期<b>規律</b>用藥物X」vs「從不用藥」，暴露定義用整個追蹤期的規律性，追蹤<b>從診斷</b>起算。", en: "Compare '<b>long-term regular</b> drug-X use' vs 'never use', defining exposure by adherence over the whole follow-up, with follow-up from diagnosis." },
    ...
    why: { zh: "『整個追蹤期都規律用藥』<b>必須活很久且沒中斷</b> → 嚴重不死時間；『用 vs 從不用』→ 適應症混淆；規律用藥者本身往往<b>更健康、生活習慣更好（healthy-adherer 效應）</b>，這種傾向常沒被測到 → 無法被測量的干擾因子。三個都中。", en: "'Regular use across all of follow-up' <b>requires surviving long and never stopping</b> → severe immortal time; 'users vs never-users' → confounding by indication; and regular adherers tend to be <b>healthier and more compliant</b>, usually unmeasured → an unmeasured confounder. All three." },
    fix: { zh: "改成<b>用藥那天才算暴露</b>（時變暴露）或<b>複製-設限-加權</b>來處理規律性與Time Zero，配<b>主動對照</b>，再對<b>健康服藥者效應（healthy-adherer effect）</b>做敏感度分析。", en: "Use <b>time-varying exposure</b> or <b>clone-censor-weight</b> for adherence/time-zero, an <b>active comparator</b>, and a sensitivity analysis for the healthy-adherer effect." }, fixMethod: "ccw" },
