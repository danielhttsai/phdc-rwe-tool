...
LOGO = os.path.join(ROOT, "frontend", "assets", "phdc-logo-horizontal.png")
...
.sa-brand-logo {{ height: 46px; width: auto; max-width: 100%; opacity: .95; }}
@media print {{
...
<div class="sa-wrap">
  ...
  <header class="sa-top">
    <h1>Lilly RWE workshop
      ...
      <span class="sa-sub">連小賴醫師都可能犯的錯：他發現「有吃藥物X 的人死亡風險少 40%」，但那 40% 是假的。四個分頁帶你把這個錯找出來、修好：Appraisal 讀出問題 · Database 找對資料 · Bias 認出偏誤 · Method 選對設計。</span>
    </h1>
  </header>
  ...
  <section class="sa-panel" id="paneBias" hidden>
    ...
    <div class="sa-scenario">換你當審查委員。先答六題認一下每種偏誤長什麼樣，再往下看幾個真實會遇到的情境，每題勾出你認為<b>存在</b>的偏誤，按「對答案」看研究者錯在哪裡、又該怎麼修正。</div>
  ...
  <section class="sa-panel" id="paneMethods" hidden>
    ...
    <div class="sa-scenario">最後一站：資料有了，那到底要用哪一種設計？先答六題，再往下用那棵樹走一次，看看你會不會走到同一個答案。</div>
